...
const ReaderManagement = () => {
	...
	const filterResults = (name) => {
		const results = dataList.filter((reader) => reader.fullName.toLowerCase().includes(name.toLowerCase()));
			...
			<div className={cx('content-container')}>
				<div className={cx('add-search-container')}>
					...
					<div className={cx('search-box')}>
						<input
							type='text'
							placeholder='Tìm kiếm độc giả theo tên'
							onChange={(e) => {
				...
				<div className={cx('table-container')}>
					<div>
						<TableWithPagination
							...
							onSignal={handleSignalFromModal}
						/>
					</div>{' '}
				</div>{' '}
			</div>
